...
export default function Home() {
  ...
  return (
    <div className="min-h-screen font-league-spartan flex flex-col items-center" style={{ background: '#2F2F32' }}>
      <main className="w-full max-w-6xl rounded-3xl shadow-2xl mt-4 sm:mt-10 mb-4 sm:mb-10 overflow-hidden" style={{ background: '#232326' }}>
        {/* Hero Section */}
        <section className="relative w-full h-[320px] sm:h-[420px] md:h-[500px] flex items-center justify-center bg-[#CBC8C8]">
          ...
          <div className="relative z-20 flex flex-col items-center justify-center w-full h-full px-4">
            <h1 style={{ color: '#fff', textShadow: '0 2px 8px rgba(30,30,30,0.5)' }} className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-center drop-shadow-lg mb-4 sm:mb-6">
              Book Top <span style={{ color: '#80F8EE', textShadow: '0 2px 8px rgba(30,30,30,0.5)', fontFamily: 'MonopolyBank, cursive' }} className="text-4xl sm:text-7xl md:text-8xl lg:text-9xl leading-tight font-extrabold">Creatives</span>. Instantly.
            </h1>
            <p style={{ color: '#fff', textShadow: '0 2px 8px rgba(30,30,30,0.5)' }} className="text-base sm:text-2xl md:text-3xl mb-6 sm:mb-8 max-w-2xl text-center font-light">Connecting clients with photographers, videographers, and creatives across South Africa.</p>
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto items-center">
              <Link href="/explore" style={{ background: '#80F8EE', color: '#171717' }} className="btn-primary text-base sm:text-lg px-6 sm:px-8 py-3 font-semibold shadow-xl hover:opacity-90 rounded-full w-full sm:w-auto text-center">Book a Creative</Link>
        ...
        <section className="px-4 sm:px-8 py-6 sm:py-8 flex justify-center">
          {homepageAd ? (
            <div style={{ background: '#80F8EE', borderColor: '#80F8EE' }} className="relative w-full max-w-3xl bg-yellow-50 border-2 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-8">
              <span style={{ background: '#80F8EE', color: '#171717' }} className="absolute top-2 left-2 sm:top-4 sm:left-4 text-xs font-bold px-3 py-1 rounded-full shadow">Sponsored</span>
              {homepageAd.image_url && (
                <img src={homepageAd.image_url} alt="Ad Logo" className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl object-cover border-4" style={{ borderColor: '#80F8EE' }} />
              )}
            ...
            <div style={{ background: '#80F8EE', borderColor: '#80F8EE' }} className="relative w-full max-w-3xl border-2 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-8">
              <span style={{ background: '#80F8EE', color: '#171717' }} className="absolute top-2 left-2 sm:top-4 sm:left-4 text-xs font-bold px-3 py-1 rounded-full shadow">Sponsored</span>
              <div className="flex-1 flex flex-col items-start">
                ...
                <Link href="/advertise" style={{ background: '#80F8EE', color: '#171717', borderColor: '#80F8EE' }} className="px-4 sm:px-6 py-2 sm:py-3 rounded-full font-semibold shadow border transition w-full sm:w-auto text-center">Advertise Now</Link>
              </div>
            </div>
          )}
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10 px-4 sm:px-8 py-10 sm:py-16 items-center">
          {/* Left: Intro */}
          <div>
            <h2 className="text-xl sm:text-3xl font-bold text-[#171717] mb-3 sm:mb-4">What is Folioo?</h2>
            <p className="text-[#171717]/80 mb-4 sm:mb-6 text-base sm:text-lg font-light">Folioo is the easiest way to find, connect, and book top creative talent for your next project or event. Whether you need a photographer, videographer, or designer, we make it seamless and fast.</p>
            <ul className="grid grid-cols-2 gap-2 sm:gap-4 mb-4 sm:mb-6">
              <li className="flex items-center gap-2 text-[#171717]/80 text-sm sm:text-base"><span style={{ background: '#80F8EE' }} className="inline-block w-3 h-3 rounded-full"></span>Vetted Creatives</li>
              <li className="flex items-center gap-2 text-[#171717]/80 text-sm sm:text-base"><span style={{ background: '#80F8EE' }} className="inline-block w-3 h-3 rounded-full"></span>Direct Messaging</li>
              <li className="flex items-center gap-2 text-[#171717]/80 text-sm sm:text-base"><span style={{ background: '#80F8EE' }} className="inline-block w-3 h-3 rounded-full"></span>Instant Booking</li>
              <li className="flex items-center gap-2 text-[#171717]/80 text-sm sm:text-base"><span style={{ background: '#80F8EE' }} className="inline-block w-3 h-3 rounded-full"></span>Showcase Portfolios</li>
            </ul>
          </div>
          {/* Right: How It Works */}
            ...
            <div className="flex flex-col gap-4 sm:gap-6">
              {steps.map((step, idx) => (
                <div key={step.title} className="flex items-center gap-3 sm:gap-4">
                  <span style={{ background: '#80F8EE', color: '#171717' }} className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full font-bold text-lg sm:text-xl shadow">{idx + 1}</span>
                  <div>
        ...
        <section className="px-4 sm:px-8 py-8 sm:py-14 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10 bg-[#F6F5F3]">
          <div className="bg-white rounded-2xl shadow p-6 sm:p-8 flex flex-col items-start">
            ...
            <Link href="/explore" style={{ background: '#80F8EE', color: '#171717', borderColor: '#80F8EE' }} className="btn-primary text-sm sm:text-md px-4 sm:px-6 py-2 font-semibold shadow rounded-full w-full sm:w-auto text-center">Book a Creative</Link>
          </div>
          <div className="bg-white rounded-2xl shadow p-6 sm:p-8 flex flex-col items-start">
            <h3 className="text-lg sm:text-xl font-bold text-[#171717] mb-2">For Creatives</h3>
            <p className="text-[#171717]/80 mb-3 sm:mb-4 text-sm sm:text-base">Showcase your portfolio, get discovered by new clients, and grow your creative business with Folioo.</p>
            <Link href="/join" style={{ background: '#80F8EE', color: '#171717', borderColor: '#80F8EE' }} className="btn-secondary text-sm sm:text-md px-4 sm:px-6 py-2 font-semibold shadow rounded-full w-full sm:w-auto text-center">Join as a Creative</Link>
          </div>
        </section>
          ...
          <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 -mx-4 sm:mx-0 px-4 sm:px-0 snap-x snap-mandatory">
            {testimonials.map((creator, idx) => (
              <div key={creator.name} className="min-w-[220px] bg-white rounded-2xl shadow-lg p-4 sm:p-6 flex flex-col items-center snap-center">
                <img src={creator.photo} alt={creator.name} className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover mb-3 sm:mb-4" style={{ border: '4px solid #80F8EE' }} />
                ...
                <Link href="#" style={{ color: '#00b3a4' }} className="font-semibold text-xs sm:text-sm mt-2 hover:underline focus:outline-none focus:ring-2 rounded">View Profile</Link>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="px-4 sm:px-8 py-6 sm:py-10 flex flex-wrap justify-between items-center gap-4 sm:gap-8 bg-white border-t border-[#E5E3E3]">
          <div className="flex flex-col items-center"><span className="text-lg sm:text-2xl font-bold" style={{ color: '#00b3a4' }}>1,200+</span><span className="text-[#171717]/70 text-xs sm:text-sm">Bookings</span></div>
          <div className="flex flex-col items-center"><span className="text-lg sm:text-2xl font-bold" style={{ color: '#00b3a4' }}>500+</span><span className="text-[#171717]/70 text-xs sm:text-sm">Creatives</span></div>
          <div className="flex flex-col items-center"><span className="text-lg sm:text-2xl font-bold" style={{ color: '#00b3a4' }}>30+</span><span className="text-[#171717]/70 text-xs sm:text-sm">Cities</span></div>
          <div className="flex flex-col items-center"><span className="text-lg sm:text-2xl font-bold" style={{ color: '#00b3a4' }}>4.9/5</span><span className="text-[#171717]/70 text-xs sm:text-sm">Avg. Rating</span></div>
        </section>
      ...
      {/* Footer */}
      <footer className="w-full max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-8 flex flex-col md:flex-row justify-between items-center rounded-t-3xl shadow-xl mb-4 sm:mb-8 gap-4 sm:gap-0" style={{ background: '#232326' }}>
        <div className="font-extrabold text-xl sm:text-2xl" style={{ color: '#00b3a4' }}>Folioo</div>
