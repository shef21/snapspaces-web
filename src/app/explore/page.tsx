    ...
    <div className="min-h-screen bg-[#F6F5F3] font-league-spartan flex flex-col items-center">
      <main className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl mt-10 mb-10 overflow-hidden px-0">
        ...
        <section className="px-4 sm:px-8 pt-12 pb-6">
          ...
          <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 -mx-4 sm:mx-0 px-4 sm:px-0 snap-x snap-mandatory">
            {profiles.filter(c => c.featured).map((creator, idx) => (
              <div 
                key={creator.id} 
                className="min-w-[280px] sm:min-w-[220px] bg-[#F6F5F3] rounded-2xl shadow-lg p-6 flex flex-col items-center border border-[#E5E3E3] snap-center"
              >
                <img src={creator.photo || '/default-profile.png'} alt={creator.name} className="w-20 h-20 rounded-full object-cover mb-4 border-4 border-yellow-300" />
                ...
                <div className="flex items-center gap-1 text-yellow-400 text-sm mb-2">★ {creator.rating} <span className="text-[#171717]/50">({creator.reviews})</span></div>
                <div className="text-[#171717]/80 font-semibold mb-2">{creator.price}</div>
                <Link 
                  href={`/creators/${creator.id}`} 
                  className="text-yellow-600 font-semibold text-sm mt-2 hover:underline focus:outline-none focus:ring-2 focus:ring-yellow-400 rounded"
                >
                  View Profile
                </Link>
              </div>
        ...
        <section className="px-8 pt-8 flex justify-center">
          <div className="relative w-full max-w-3xl bg-yellow-50 border-2 border-yellow-300 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-6 p-6 md:p-8">
            <span className="absolute top-4 left-4 bg-yellow-300 text-[#171717] text-xs font-bold px-3 py-1 rounded-full shadow">Sponsored</span>
            <img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=200&q=80" alt="Ad Logo" className="w-20 h-20 rounded-xl object-cover border-4 border-yellow-200 shadow-md" />
            <div className="flex-1 flex flex-col items-start">
              ...
              <button className="px-6 py-3 rounded-full bg-yellow-300 hover:bg-yellow-400 text-[#171717] font-semibold shadow border border-yellow-300 transition focus:outline-none focus:ring-2 focus:ring-yellow-400 active:scale-95" aria-label="Get Featured">
                Get Featured
              </button>
            </div>
          </div>
        ...
        <section className="px-4 sm:px-8 py-6 bg-[#F6F5F3] border-t border-b border-[#E5E3E3] flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <input
              ...
              className="w-full px-5 py-3 rounded-full border border-[#E5E3E3] bg-white text-[#171717] focus:outline-none focus:ring-2 focus:ring-yellow-300 text-base shadow"
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <select 
                className="w-full sm:w-auto px-4 py-3 rounded-full border border-[#E5E3E3] bg-white text-[#171717] text-base shadow focus:outline-none focus:ring-2 focus:ring-yellow-300"
          ...
          <Tooltip text="Apply your selected filters">
            <button 
              className="w-full sm:w-auto px-6 py-3 rounded-full bg-yellow-300 hover:bg-yellow-400 text-[#171717] font-semibold shadow border border-yellow-300 transition focus:outline-none focus:ring-2 focus:ring-yellow-400"
        ...
        <section className="px-4 sm:px-8 py-12">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <Tooltip text="Clear all filters and show all creatives">
                <button className="px-6 py-3 rounded-full bg-yellow-300 hover:bg-yellow-400 text-[#171717] font-semibold shadow border border-yellow-300 transition focus:outline-none focus:ring-2 focus:ring-yellow-400 active:scale-95" onClick={() => { setSearchText(''); setSelectedSpecialty('All'); setSelectedLocation('All'); setSelectedRating('All'); setSelectedPrice('All'); }} aria-label="Reset Filters">
                  Reset Filters
                </button>
              </Tooltip>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
              {filteredProfiles.map((creator, idx) => (
                <div key={creator.id} className="bg-[#F6F5F3] rounded-2xl shadow-lg p-6 flex flex-col items-center border border-[#E5E3E3]">
                  <img src={creator.photo || '/default-profile.png'} alt={creator.name} className="w-20 h-20 rounded-full object-cover mb-4 border-4 border-yellow-300" />
                  <div className="font-bold text-[#171717] text-lg mb-1">{creator.name || 'Unnamed'}</div>
                  <div className="text-[#171717]/70 text-sm mb-2">{(creator.specialties || []).join(', ')}</div>
                  <div className="text-[#171717]/60 text-xs mb-2">{creator.location}</div>
